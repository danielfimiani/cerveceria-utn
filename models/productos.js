...
const create = (obj,uid) =>
  pool
    ...
    .query("select   c.txt_categoria,txt_nombre,txt_desc, imp_precio,sn_especial,image,case when p.sn_habilitado= 1 then 'SI' else 'NO' end as 'sn_habilitado',p.ts_create, p.ts_update from ??  p inner join categorias c on p.id_categoria = c.id_categoria"[
      T_PRODUCTO])
    .then((result) => result)
    .catch((e) =>  e);
